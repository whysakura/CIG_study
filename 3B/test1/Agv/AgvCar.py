...
class Station(object):
    ...
    def __init__(self, index, markNumber, dir):
        self.index = index                  # 站点序号
        self.markNumber = markNumber        # 站点mark号
        self.dir = dir                      # 站点方向, 1-正向, 2-反向

class Branch(object):

    def __init__(self, index, markNumber, dir):
        self.index = index                  # 分岔点序号
        self.markNumber = markNumber        # 分岔点mark号
        self.dir = dir                      # 分岔点方向, 1-正向, 2-反向
...
class AgvCar(object):
        ...
        with open(self.agvfilename, 'rb') as fileData:
            ...
            for i in range(200):
                ...
                if stationMarkNumber == 0:
                    break
                self.StationDict[i+1] = Station(i+1, stationMarkNumber, stationDir)
            fileData.close()
        ...
        with open(self.agvfilename, 'rb') as fileData:
            ...
            for i in range(200):
                ...
                if branchMarkNumber == 0:
                    break
                self.BranchDict[branchMarkNumber] = Branch(i+1, branchMarkNumber, branchDir)
            fileData.close()
    ...
    def ReadFromTrun(self):                 # 从自制的turn.csv文件中读取数据
            ...
            for line in reader:
                if reader.line_num == 1:
                    ...
                    for i in range(len(goals)):
                        d[int(goals[i])] = int(line[i+1])
                    self.Turn[int(line[0])] = d
            fileData.close()

    def ReadDistance(self):                 # 从自制的distance.csv文件中读取数据
    ...
    def FindNextPoint(self, start, end):    # 搜索起点至终点的下一点
        for key, value in self.BranchDict.items():      # 遍历所有分岔点
            if start == value.markNumber:               # 如果起点是分岔点
